...
On Android, install the app from the browser menu. Chrome is currently the recommended browser for installation. Samsung Internet may show a Google Play Protect warning for some PWAs on recent Android versions; this appears to be related to the WebAPK generated by Samsung Internet rather than to this static web app. When launched from the home screen, it should open without the browser address bar and browser navigation controls. The Android system navigation bar can still remain visible, depending on the device and browser.

When files are changed, the service worker cache version in `service-worker.js` may need to be updated so installed copies refresh cleanly. If the PWA manifest changes, the manifest link in `index.html` may also be versioned to force browsers to fetch the new metadata.
